...
class TemplateCustomizer {
    ...
    setTheme(themeName, updateStorage = true, cb = null) {
        ...
        this._loadStylesheets({ [themeUrl]: document.querySelector(".template-customizer-theme-css") },
            cb || (() => {}));
    ...
    update() {
        ...
        const hasHorizontalMenu =
            !!document.querySelector(".layout-menu-horizontal.menu, .layout-menu-horizontal .menu");
    ...
    _setup(_container = document) {
        ...
        if (!this._hasControls("themes")) {
            ...
            this.settings.availableThemes.forEach(theme => {
                const themeEl = this._getElementFromString(
                    ...
                    theme.name}" value="${theme.name}"><label class="form-check-label" for="themeRadios${theme.name}">${
                    theme.title}</label></div></div>`
                );
        ...
        if (
            !this._hasControls("layoutType layoutNavbarFixed layoutFooterFixed layoutMenuFlipped showDropdownOnHover",
                true)
        ) {
            ...
            this._controls.showDropdownOnHover =
                this.container.querySelector(".template-customizer-showDropdownOnHover");
            ...
            this._insertStylesheet(
                ...
                this.pathResolver(
                    ...
                    this.settings.cssFilenamePattern.replace(
                        ...
                        theme
                        ? theme
                        : `theme-default${this.settings.style !== "light" ? `-${this.settings.style}` : ""}`
                    )
                )
            );
        }
