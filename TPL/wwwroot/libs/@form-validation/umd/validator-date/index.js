(function(global, factory) {
    typeof exports === "object" && typeof module !== "undefined"
        ...
        : (global = typeof globalThis !== "undefined" ? globalThis : global || self, (global.FormValidation =
            ...
            global.FormValidation.validators || {}, global.FormValidation.validators.date =
    ...
    (function(core) {
        ...
        var parseDate = function(input, inputFormat, separator) {
            ...
            const d = new Date(parseInt(dateSection[yearIndex], 10),
                parseInt(dateSection[monthIndex], 10) - 1,
                parseInt(dateSection[dayIndex], 10));
            const amPmSection = sections.length > 2 ? sections[2] : null;
            if (sections.length > 1) {
                const timeSection = sections[1].split(":");
                const h = timeSection.length > 0 ? parseInt(timeSection[0], 10) : 0;
        ...
        var formatDate = function(input, inputFormat) {
            ...
            const dd = d < 10 ? "0".concat(d) : d;
            const m = input.getMonth() + 1;
            const mm = m < 10 ? "0".concat(m) : m;
            const yy = "".concat(input.getFullYear()).substr(2);
            const yyyy = input.getFullYear();
            const h = input.getHours() % 12 || 12;
            const hh = h < 10 ? "0".concat(h) : h;
            const H = input.getHours();
            const HH = H < 10 ? "0".concat(H) : H;
            const M = input.getMinutes();
            const MM = M < 10 ? "0".concat(M) : M;
            const s = input.getSeconds();
            const ss = s < 10 ? "0".concat(s) : s;
            var replacer = {
                H: "".concat(H),
                HH: "".concat(HH),
                M: "".concat(M),
            ...
            return dateFormat.replace(/d{1,4}|m{1,4}|yy(?:yy)?|([HhMs])\1?|"[^"]*"|'[^']*'/g,
                function(match) {
            ...
            return {
                validate: function(input) {
                    ...
                    if (timeFormat) {
                        ...
                        var h = hms.length > 0
                            ? (hms[0].length <= 2 && /^\d+$/.test(hms[0]) ? parseInt(hms[0], 10) : -1)
                            : 0;
                        var m = hms.length > 1
                            ? (hms[1].length <= 2 && /^\d+$/.test(hms[1]) ? parseInt(hms[1], 10) : -1)
                            : 0;
                        var s = hms.length > 2
                            ? (hms[2].length <= 2 && /^\d+$/.test(hms[2]) ? parseInt(hms[2], 10) : -1)
                            : 0;
                        if (h === -1 || m === -1 || s === -1) {
                            return invalidResult;
                        }
